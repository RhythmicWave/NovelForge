...
from app.services.ai.llm_service import build_chat_model
from app.services.ai.model_builder import build_model_from_json_schema
from app.services.ai.instruction_validator import (
    validate_instruction, 
    apply_instruction,
    format_validation_errors
)
from app.services.ai.prompt_builder import build_user_task_prompt
                    ...
                    if instruction:
                        ...
                        try:
                            ...
                            if instruction.get('op') == 'done':
                                logger.info("[Done 指令] 收到 done 指令，开始完整性校验...")
                                try:
                                    # 使用 Pydantic 验证完整数据
                                    validated_model = DynamicModel(**collected_data)
                                    
                                    # 校验通过，生成完成
                                    logger.info("[Done 指令] ✅ 完整性校验通过，生成成功！")
                                    ...
                                    logger.warning(f"完整性校验失败，尝试自动修复: {error_msg}")
...
"""
                            ...
                            # 指令校验失败，累积错误
                            logger.warning(f"指令校验失败: {e}, 指令: {instruction}")
                            ...
                            # 如果累积失败过多，立即中断并反馈
                            if len(failed_instructions) >= 3:
                                logger.warning(f"连续失败 {len(failed_instructions)} 次，中断生成并反馈")
                                should_break_stream = True
...
"""
            ...
            try:
                ...
                if missing_optional_fields:
                    ...
                    yield {
                        "type": "warning",
                        "text": f"⚠️ 生成可能被截断（LLM 未发送 done 指令）。以下字段缺失：{', '.join(missing_optional_fields)}。\n\n建议：增加 max_tokens 参数以生成完整内容。"
                    }
                    # 尝试修复
...
"""
                ...
                if attempt == 0:
                    yield {
                        "type": "error",
                        "text": f"⚠️ 生成被截断，可能是 max_tokens 设置过小。缺失字段：{error_msg}\n\n建议：增加 max_tokens 参数（当前可能不足）"
                    }
